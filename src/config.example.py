...
STATUS_LED_PIN = 2  # Addressable RGB data pin; S3 onboard RGB: 48.
STATUS_LED_TYPE = "rgb"  # Single NeoPixel/WS2812; "plain" for a one-color LED.
WEB_DEBUG = False
WEB_SEND_DEBUG = False
UPDATE_CHECK_HOUR = 4
UPDATE_AUTO_INSTALL = True  # Checked GitHub releases; only while valves are idle.
UPDATE_GITHUB_REPO = "okayaleh/ESP32-watering-rebuild"
UPDATE_MANIFEST_PATH = "build/manifest.json"
UPDATE_TIMEOUT_SEC = 120  # Absolute deadline per HTTPS transfer, including DNS/TLS.
UPDATE_BASE_URL = ""  # Empty uses GitHub directly; optional trusted HTTP LAN mirror.
